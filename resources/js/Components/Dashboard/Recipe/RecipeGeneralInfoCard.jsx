...
export default function RecipeGeneralInfoCard({
  ...
  const nutritionOptions = nutritionTags.map((tag, i) => mapToOption(tag, i + 1));
...
const getSelectedObjects = (ids, options) =>
    ...
    <div className="rounded-xl border bg-white p-6 shadow-sm">
      ...
      <div className="space-y-5">
        ...
        <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-gray-300 p-6 text-center hover:border-gray-400">
          ...
          <div className="text-sm font-medium text-gray-700">Upload Foto</div>
  ...
  { label: 'Jumlah Porsi', name: 'cook', placeholder: 'Jumlah Porsi', unit: 'porsi' },
          ...
          <div key={name} className="relative">
            ...
            />
            {unit && <span className="absolute right-4 top-9 text-sm text-gray-700">{unit}</span>}
            {errors[name] && (
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700">Kategori Hidangan</label>
          <select
          ...
          >
            {['', 'Hidangan Pembuka', 'Sarapan', 'Hidangan Utama', 'Hidangan Penutup', 'Minuman', 'Pelengkap', 'Cemilan', 'Hidangan Berkuah'].map((option) => (
              <option key={option} value={option}>
                {option === '' ? 'Pilih Kategori Hidangan' : option}
              </option>
            ))}
          </select>
          {errors.kategori_hidangan && (
            <div className="text-sm text-red-500 mt-1">{errors.kategori_hidangan}</div>
          )}
        </div>

        {/* Select: Metode Memasak */}
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700">Metode Memasak</label>
          <select
            name="metode_memasak"
            ...
            className="w-full rounded-md border px-3 py-2 text-sm shadow-sm"
          >
            {['', 'Merebus', 'Menggoreng', 'Mengukus', 'Memanggang', 'Menumis', 'Merebus Perlahan', 'Menggulai (Stewing)', 'Menggoreng Cepat(Stir Frying)'].map((option) => (
              <option key={option} value={option}>
                {option === '' ? 'Pilih Metode Memasak' : option}
              </option>
            ))}
          </select>
          {errors.metode_memasak && (
            <div className="text-sm text-red-500 mt-1">{errors.metode_memasak}</div>
          )}
        </div>
  ...
  selectedValues={getSelectedObjects(data.health_tags || [], healthOptions)}
...
<MultiSelectCheckbox
  ...
  selectedValues={getSelectedObjects(data.allergy_tags || [], allergyOptions)}
...
<MultiSelectCheckbox
  ...
  selectedValues={getSelectedObjects(data.nutrition_tags || [], nutritionOptions)}
  onChange={handleTagChange('nutrition_tags', nutritionOptions)}
/>
...
<MultiSelectCheckbox
  ...
  name="diet_tags"
  options={dietOptions}
  selectedValues={getSelectedObjects(data.diet_tags || [], dietOptions)}
